...
void runCommand(char* command) {
  ...
  if (strncmp(command, "type ", 5)) {
    ...
    interrupt(0x21, 0x2, buf, 2, 0);
    interrupt(0x21, 0x98, buf, 0, 0);
  } else if (strncmp(command, "create ", 7)) {
    createFile(command + 7, buf);
  } else if (strncmp(command, "kill ", 5)) {
    fsize = atoi(command[5]);
    interrupt(0x21, 0x0, "Killing process: ", 0, 0);
    interrupt(0x21, 0x99, fsize, 0, 0);
    interrupt(0x21, 0x0, "\r\n", 0, 0);
    interrupt(0x21, 0x9, fsize, 0, 0);
  } else if (strncmp(command, "execforeground ", 15)) {
    interrupt(0x21, 0x97, command + 15, 0, 0);
  } else if (strncmp(command, "clear", 5)) {
    interrupt(0x21, 0x96, 0, 0, 0);
  } else if (strncmp(command, "help", 4)) {
    interrupt(0x21, 0x3, command, buf, 0);
    interrupt(0x21, 0x0, buf, 0, 0);
  }
